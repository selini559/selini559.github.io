...
![image-20220917183658408](C:\Users\asus\AppData\Roaming\Typora\typora-user-images\image-20220917183658408.png)
...
1. 当v-for遍历的数组发生变化时，(一定是原数组得发生变化) , 会触发v-for的遍历更新
...
![image-20220917193452732](C:\Users\asus\Desktop\blog\docs\md\vue02\image-20220917193452732.png)

### 🏷️8.2 根元素没变, 属性改变

**元素复用, 只更新属性**

![image-20220917193534223](C:\Users\asus\Desktop\blog\docs\md\vue02\image-20220917193534223.png)
...
![image-20220917193723254](C:\Users\asus\Desktop\blog\docs\md\vue02\image-20220917193723254.png)
...
![image-20220917193923908](C:\Users\asus\Desktop\blog\docs\md\vue02\image-20220917193923908.png)
...
![image-20220917194746425](C:\Users\asus\Desktop\blog\docs\md\vue02\image-20220917194746425.png)
...
**答：**在js中，渲染真是`DOM`的开销是非常大的，比如我们修改了某个数据，如果直接渲染到真实`DOM` ,会引起整个`DOM`树重绘和重排。那么有没有可能实现只更新我们修改的那一小块`DOM`二不要更新整个`DOM`呢?此时我们就需要先根据真实`DOM`生成虚拟`DOM`，当虚拟`DOM`某个节点的数据改变后会生成有一个新的`VNode`，然后新的`VNode`和旧的`VNode`作比较，发现有不一样的地方就直接修改在真实DOM上，然后旧的`VNode`的值为新的`VNode`；（<font color='red'>找不同！</font>）

## 🏝️10. 数组方法归纳

会改变原数组，v-for可以监测到并更新页面的方法：
